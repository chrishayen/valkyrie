...
build:
	@mkdir -p $(BUILD_DIR)
	$(ODIN) build . -o:speed -extra-linker-flags:"-static -L/usr/local/lib -lwolfssl -lm -lpthread" -out:$(BUILD_DIR)/$(BINARY)

build-nix:
	@mkdir -p $(BUILD_DIR)
	$(ODIN) build . -o:speed -extra-linker-flags:"-static -lwolfssl -lm -lpthread" -out:$(BUILD_DIR)/$(BINARY)
...
build-arm64:
	@mkdir -p $(BUILD_DIR)
	$(ODIN) build . -o:speed -target:linux_arm64 -extra-linker-flags:"--target=aarch64-linux-gnu -static -lwolfssl -lm -lpthread" -out:$(BUILD_DIR)/$(BINARY)-arm64
