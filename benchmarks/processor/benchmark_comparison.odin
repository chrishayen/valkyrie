...
Lockless_Handler :: struct {
	process:       proc(h: ^Lockless_Handler, item: valkyrie.Processor_Work_Item) -> valkyrie.Work_Result,
	process_count: u64,
}

lockless_process :: proc(h: ^Lockless_Handler, item: valkyrie.Processor_Work_Item) -> valkyrie.Work_Result {
...
run_lockless_benchmark :: proc(num_workers: int, num_items: int) -> f64 {
	...
	p := new(valkyrie.Processor(Lockless_Handler))
