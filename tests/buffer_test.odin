...
test_buffer_init :: proc(t: ^testing.T) {
	...
	{
		rb, ok := http.buffer_init(1024)
		defer http.buffer_destroy(&rb)
...
test_buffer_is_empty :: proc(t: ^testing.T) {
	rb, ok := http.buffer_init(64)
	defer http.buffer_destroy(&rb)
	testing.expect(t, ok)

	// Initially empty
	testing.expect(t, http.buffer_is_empty(&rb), "new buffer should be empty")

	// Not empty after write
	data := []u8{1, 2, 3}
	http.buffer_write(&rb, data)
	testing.expect(t, !http.buffer_is_empty(&rb), "buffer should not be empty after write")

	// Empty after reading all data
	buf := make([]u8, 3)
	defer delete(buf)
	http.buffer_read(&rb, buf)
	testing.expect(t, http.buffer_is_empty(&rb), "buffer should be empty after reading all data")
}

@(test)
test_buffer_is_full :: proc(t: ^testing.T) {
	rb, ok := http.buffer_init(4)
	defer http.buffer_destroy(&rb)
	testing.expect(t, ok)

	// Initially not full
	testing.expect(t, !http.buffer_is_full(&rb), "new buffer should not be full")

	// Full after writing to capacity
	data := []u8{1, 2, 3, 4}
	written := http.buffer_write(&rb, data)
	testing.expect_value(t, written, 4)
	testing.expect(t, http.buffer_is_full(&rb), "buffer should be full after writing to capacity")

	// Not full after partial read
	buf := make([]u8, 2)
	defer delete(buf)
	http.buffer_read(&rb, buf)
	testing.expect(t, !http.buffer_is_full(&rb), "buffer should not be full after partial read")
}
...
test_buffer_write_and_read :: proc(t: ^testing.T) {
	...
	written := http.buffer_write(&rb, write_data)
	testing.expect_value(t, written, 5)
	testing.expect_value(t, http.buffer_available_read(&rb), 5)

	read_data := make([]u8, 5)
	defer delete(read_data)
	read := http.buffer_read(&rb, read_data)
...
test_buffer_write_partial :: proc(t: ^testing.T) {
	rb, ok := http.buffer_init(10)
	defer http.buffer_destroy(&rb)
	testing.expect(t, ok)

	// Fill buffer partially
	data1 := []u8{1, 2, 3, 4, 5}
	written := http.buffer_write(&rb, data1)
	testing.expect_value(t, written, 5)

	// Try to write more than available space
	data2 := []u8{6, 7, 8, 9, 10, 11, 12}
	written = http.buffer_write(&rb, data2)
	testing.expect_value(t, written, 5)
	testing.expect(t, http.buffer_is_full(&rb), "buffer should be full")
}

@(test)
test_buffer_wraparound :: proc(t: ^testing.T) {
	rb, ok := http.buffer_init(8)
	defer http.buffer_destroy(&rb)
	testing.expect(t, ok)

	// Write initial data
	data1 := []u8{1, 2, 3, 4, 5}
	http.buffer_write(&rb, data1)
	...
	result := make([]u8, 8)
	defer delete(result)
	read := http.buffer_read(&rb, result)
...
test_buffer_peek :: proc(t: ^testing.T) {
	...
	// Write data
	write_data := []u8{1, 2, 3, 4, 5}
	http.buffer_write(&rb, write_data)

	// Peek at data
	peek_buf := make([]u8, 3)
	defer delete(peek_buf)
	peeked := http.buffer_peek(&rb, peek_buf)
	...
	read := http.buffer_read(&rb, read_buf)
	testing.expect_value(t, read, 5)
	for i in 0..<5 {
...
test_buffer_consume :: proc(t: ^testing.T) {
	...
	// Consume some bytes
	consumed := http.buffer_consume(&rb, 3)
...
test_buffer_clear :: proc(t: ^testing.T) {
	...
	// Clear buffer
	http.buffer_clear(&rb)

	testing.expect(t, http.buffer_is_empty(&rb), "buffer should be empty after clear")
	testing.expect_value(t, rb.size, 0)
	testing.expect_value(t, rb.read_pos, 0)
	testing.expect_value(t, rb.write_pos, 0)
}
...
test_buffer_grow :: proc(t: ^testing.T) {
	...
	written := http.buffer_write(&rb, more_data)
...
test_buffer_available_space :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, http.buffer_available_write(&rb), 10)
	testing.expect_value(t, http.buffer_available_read(&rb), 0)

	// Write some data
	data := []u8{1, 2, 3, 4, 5}
	http.buffer_write(&rb, data)

	testing.expect_value(t, http.buffer_available_write(&rb), 5)
	testing.expect_value(t, http.buffer_available_read(&rb), 5)
	...
	testing.expect_value(t, http.buffer_available_write(&rb), 7)
	testing.expect_value(t, http.buffer_available_read(&rb), 3)
}
...
test_buffer_write_empty_data :: proc(t: ^testing.T) {
	...
	// Write empty slice
	empty := []u8{}
	written := http.buffer_write(&rb, empty)
	testing.expect_value(t, written, 0)
	testing.expect(t, http.buffer_is_empty(&rb), "buffer should remain empty")
}
