...
test_connection_init :: proc(t: ^testing.T) {
	// Positive test: valid fd
	{
		...
		conn, ok := http.connection_init(pipes[0])
		...
		testing.expect_value(t, conn.state, http.Connection_State.New)
	...
	{
		_, ok := http.connection_init(linux.Fd(-1))
...
test_connection_destroy :: proc(t: ^testing.T) {
	...
	http.connection_destroy(&conn)

	testing.expect_value(t, conn.fd, linux.Fd(-1))
	testing.expect_value(t, conn.state, http.Connection_State.Closed)
...
test_connection_set_nonblocking :: proc(t: ^testing.T) {
	...
	// Set non-blocking
	success := http.connection_set_nonblocking(&conn)
...
test_connection_queue_write :: proc(t: ^testing.T) {
	...
	// Queue some data
	data := []u8{1, 2, 3, 4, 5}
	queued := http.connection_queue_write(&conn, data)
	testing.expect_value(t, queued, 5)

	// Verify data is in buffer
	testing.expect(t, http.connection_has_write_pending(&conn), "should have pending write")
	testing.expect_value(t, http.buffer_available_read(&conn.write_buffer), 5)
}
...
test_connection_write_pending :: proc(t: ^testing.T) {
	...
	conn, ok := http.connection_init(pipes[1])
	defer http.connection_destroy(&conn)
	testing.expect(t, ok)

	// Queue data
	write_data := []u8{1, 2, 3, 4, 5}
	http.connection_queue_write(&conn, write_data)

	// Write pending data
	written := http.connection_write_pending(&conn)
	testing.expect(t, written == 5, "should write all queued data")

	// Verify no more pending writes
	testing.expect(t, !http.connection_has_write_pending(&conn), "should have no pending writes")
...
test_connection_read_available :: proc(t: ^testing.T) {
	...
	// Read available data
	read := http.connection_read_available(&conn)
	testing.expect(t, read == 8, "should read all available data")

	// Verify data is in buffer
	testing.expect_value(t, http.connection_available_data(&conn), 8)

	// Read from buffer
	read_buf := make([]u8, 8)
	defer delete(read_buf)
	http.connection_read_data(&conn, read_buf)
...
test_connection_read_available_closed :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, conn.state, http.Connection_State.Closing)
}
...
test_connection_peek_data :: proc(t: ^testing.T) {
	...
	http.connection_read_available(&conn)

	// Peek at data
	peek_buf := make([]u8, 3)
	defer delete(peek_buf)
	peeked := http.connection_peek_data(&conn, peek_buf)
	...
	testing.expect_value(t, http.connection_available_data(&conn), 5)
...
test_connection_is_idle :: proc(t: ^testing.T) {
	...
	testing.expect(t, !http.connection_is_idle(&conn, 100 * time.Millisecond), "should not be idle")

	// Wait a bit
	time.sleep(150 * time.Millisecond)

	// Should be idle now
	testing.expect(t, http.connection_is_idle(&conn, 100 * time.Millisecond), "should be idle")

	// Mark active
	http.connection_mark_active(&conn)

	// Should not be idle anymore
	testing.expect(t, !http.connection_is_idle(&conn, 100 * time.Millisecond), "should not be idle after mark_active")
}
...
test_connection_close :: proc(t: ^testing.T) {
	...
	testing.expect(t, !http.connection_is_closed(&conn), "should not be closed initially")

	// Close connection
	http.connection_close(&conn)

	testing.expect_value(t, conn.state, http.Connection_State.Closing)
	testing.expect(t, !http.connection_is_closed(&conn), "Closing state is not fully closed")

	// Destroy to fully close
	http.connection_destroy(&conn)
	testing.expect(t, http.connection_is_closed(&conn), "should be closed after destroy")
}
...
test_connection_queue_write_buffer_growth :: proc(t: ^testing.T) {
	...
	queued := http.connection_queue_write(&conn, large_data)
...
test_connection_state_transitions :: proc(t: ^testing.T) {
	...
	conn.state = .Active
	testing.expect_value(t, conn.state, http.Connection_State.Active)
