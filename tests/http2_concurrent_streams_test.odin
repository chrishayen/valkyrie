...
test_max_concurrent_streams_enforcement :: proc(t: ^testing.T) {
	...
	stream_count := http.connection_stream_count(&handler.conn)
	...
	final_count := http.connection_stream_count(&handler.conn)
...
test_multiple_concurrent_streams :: proc(t: ^testing.T) {
	...
	testing.expect(t, http.protocol_handler_needs_write(&handler), "Should have responses")
...
test_per_stream_flow_control_isolation :: proc(t: ^testing.T) {
	...
	stream1, _ := http.connection_create_stream(&handler.conn, 1)
	stream2, _ := http.connection_create_stream(&handler.conn, 3)
	...
	http.protocol_handler_process_data(&handler, headers2)
	...
	final_stream1, _ := http.connection_get_stream(&handler.conn, 1)
	final_stream2, _ := http.connection_get_stream(&handler.conn, 3)
...
test_stream_error_isolation :: proc(t: ^testing.T) {
	...
	http.protocol_handler_get_write_data(&handler)
	http.protocol_handler_consume_write_data(&handler, 10000)
	...
	http.protocol_handler_process_data(&handler, bad_data_frame)

	// Should send RST_STREAM for stream 3
	testing.expect(t, http.protocol_handler_needs_write(&handler), "Should send RST_STREAM")

	error_response := http.protocol_handler_get_write_data(&handler)
	...
	testing.expect(t, !http.connection_is_closing(&handler.conn), "Connection should still be active")
	...
	_, stream1_exists = http.connection_get_stream(&handler.conn, 1)
	_, stream3_exists = http.connection_get_stream(&handler.conn, 3)
	_, stream5_exists = http.connection_get_stream(&handler.conn, 5)
...
test_connection_flow_control_shared :: proc(t: ^testing.T) {
	...
	stream1, _ := http.connection_create_stream(&handler.conn, 1)
	stream2, _ := http.connection_create_stream(&handler.conn, 3)
	stream3, _ := http.connection_create_stream(&handler.conn, 5)
	...
	final_stream1, found1 := http.connection_get_stream(&handler.conn, 1)
	final_stream2, found2 := http.connection_get_stream(&handler.conn, 3)
	final_stream3, found3 := http.connection_get_stream(&handler.conn, 5)
