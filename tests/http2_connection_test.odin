...
test_http2_connection_init :: proc(t: ^testing.T) {
	conn, ok := http.connection_init(true)  // Server connection
	defer http.connection_destroy(&conn)
...
test_http2_connection_init_client :: proc(t: ^testing.T) {
	conn, ok := http.connection_init(false)  // Client connection
	defer http.connection_destroy(&conn)
...
test_http2_connection_handle_preface_valid :: proc(t: ^testing.T) {
	conn, _ := http.connection_init(true)
	defer http.connection_destroy(&conn)
	...
	err := http.connection_handle_preface(&conn, preface)
...
test_http2_connection_handle_preface_invalid :: proc(t: ^testing.T) {
	...
	err := http.connection_handle_preface(&conn, invalid_preface)
...
test_http2_connection_handle_settings :: proc(t: ^testing.T) {
	...
	settings_frame, _ := http.settings_build_frame(&conn.settings)
	defer delete(settings_frame.settings)

	err := http.connection_handle_settings(&conn, &settings_frame)
...
test_http2_connection_handle_settings_ack :: proc(t: ^testing.T) {
	...
	err := http.connection_handle_settings(&conn, &ack_frame)
...
test_http2_connection_create_stream :: proc(t: ^testing.T) {
	...
	stream, err := http.connection_create_stream(&conn, 1)
	...
	testing.expect(t, http.connection_stream_count(&conn) == 1, "Should have 1 stream")
}
...
test_http2_connection_create_duplicate_stream :: proc(t: ^testing.T) {
	...
	// Create first stream
	http.connection_create_stream(&conn, 1)

	// Try to create duplicate
	_, err := http.connection_create_stream(&conn, 1)
...
test_http2_connection_get_stream :: proc(t: ^testing.T) {
	...
	// Get stream
	stream, found := http.connection_get_stream(&conn, 1)
	...
	_, found2 := http.connection_get_stream(&conn, 999)
...
test_http2_connection_remove_stream :: proc(t: ^testing.T) {
	...
	removed := http.connection_remove_stream(&conn, 1)
	testing.expect(t, removed == true, "Should remove stream")
	testing.expect(t, http.connection_stream_count(&conn) == 0, "Should have 0 streams")
}
...
test_http2_connection_handle_ping :: proc(t: ^testing.T) {
	...
	err := http.connection_handle_ping(&conn, &ping_frame, false)
...
test_http2_connection_handle_goaway :: proc(t: ^testing.T) {
	...
	// Create some streams
	http.connection_create_stream(&conn, 1)
	http.connection_create_stream(&conn, 3)
	http.connection_create_stream(&conn, 5)

	// Send GOAWAY with last_stream_id = 3
	goaway_frame := http.Goaway_Frame{
		header = http.Frame_Header{
			length = 8,
			type = .GOAWAY,
			flags = 0,
	...
	err := http.connection_handle_goaway(&conn, &goaway_frame)
	...
	testing.expect(t, http.connection_stream_count(&conn) == 2, "Should have 2 streams left")

	_, found1 := http.connection_get_stream(&conn, 1)
	testing.expect(t, found1 == true, "Stream 1 should exist")

	_, found3 := http.connection_get_stream(&conn, 3)
	testing.expect(t, found3 == true, "Stream 3 should exist")

	_, found5 := http.connection_get_stream(&conn, 5)
...
test_http2_connection_send_goaway :: proc(t: ^testing.T) {
	...
	err := http.connection_send_goaway(&conn, .NO_ERROR)
...
test_http2_connection_flow_control :: proc(t: ^testing.T) {
	...
	// Consume some window
	err := http.connection_consume_window(&conn, 1000)
	...
	// Update window
	err = http.connection_update_window(&conn, 2000)
...
test_http2_connection_flow_control_overflow :: proc(t: ^testing.T) {
	...
	// Try to overflow
	err := http.connection_update_window(&conn, 200)
...
test_http2_connection_flow_control_underflow :: proc(t: ^testing.T) {
	...
	err := http.connection_consume_window(&conn, 200)
...
test_http2_connection_state_checks :: proc(t: ^testing.T) {
	...
	testing.expect(t, http.connection_is_active(&conn) == false, "Should not be active initially")
	testing.expect(t, http.connection_is_closing(&conn) == false, "Should not be closing initially")

	conn.state = .Active
	testing.expect(t, http.connection_is_active(&conn) == true, "Should be active")
	testing.expect(t, http.connection_is_closing(&conn) == false, "Should not be closing")

	conn.state = .Going_Away
	testing.expect(t, http.connection_is_active(&conn) == false, "Should not be active")
	testing.expect(t, http.connection_is_closing(&conn) == true, "Should be closing")
}
...
test_http2_connection_can_create_stream :: proc(t: ^testing.T) {
	...
	testing.expect(t, http.connection_can_create_stream(&conn) == false, "Cannot create before Active")

	conn.state = .Active
	testing.expect(t, http.connection_can_create_stream(&conn) == true, "Can create when Active")

	conn.state = .Going_Away
	testing.expect(t, http.connection_can_create_stream(&conn) == false, "Cannot create when Going_Away")
}
...
test_http2_connection_stream_limit :: proc(t: ^testing.T) {
	...
	http.settings_apply_local(&conn.settings, .MAX_CONCURRENT_STREAMS, 2)
	...
	// Try to exceed limit
	_, err := http.connection_create_stream(&conn, 5)
