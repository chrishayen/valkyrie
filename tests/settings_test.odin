...
test_settings_init :: proc(t: ^testing.T) {
	ctx := http2.settings_init()
	defer http2.settings_destroy(&ctx)
...
test_settings_apply_local :: proc(t: ^testing.T) {
	...
	err := http2.settings_apply_local(&ctx, .HEADER_TABLE_SIZE, 8192)
...
test_settings_apply_remote :: proc(t: ^testing.T) {
	...
	err := http2.settings_apply_remote(&ctx, .MAX_CONCURRENT_STREAMS, 200)
	testing.expect(t, err == .None, "Should apply successfully")
...
test_settings_enable_push_valid :: proc(t: ^testing.T) {
	...
	// Disable push
	err := http2.settings_apply_remote(&ctx, .ENABLE_PUSH, 0)
	testing.expect(t, err == .None, "Should apply successfully")
	testing.expect(t, ctx.remote_enable_push == false, "Should disable push")

	// Enable push
	err = http2.settings_apply_remote(&ctx, .ENABLE_PUSH, 1)
	testing.expect(t, err == .None, "Should apply successfully")
...
test_settings_enable_push_invalid :: proc(t: ^testing.T) {
	...
	err := http2.settings_apply_remote(&ctx, .ENABLE_PUSH, 2)
...
test_settings_initial_window_size_valid :: proc(t: ^testing.T) {
	...
	// Valid window size
	err := http2.settings_apply_local(&ctx, .INITIAL_WINDOW_SIZE, 1000000)
	testing.expect(t, err == .None, "Should apply successfully")
...
test_settings_initial_window_size_too_large :: proc(t: ^testing.T) {
	...
	// Too large (> 2^31 - 1)
	err := http2.settings_apply_remote(&ctx, .INITIAL_WINDOW_SIZE, 0x80000000)
...
test_settings_max_frame_size_valid :: proc(t: ^testing.T) {
	...
	// Valid frame size
	err := http2.settings_apply_local(&ctx, .MAX_FRAME_SIZE, 32768)
	testing.expect(t, err == .None, "Should apply successfully")
...
test_settings_max_frame_size_too_small :: proc(t: ^testing.T) {
	...
	// Too small (< 2^14)
	err := http2.settings_apply_remote(&ctx, .MAX_FRAME_SIZE, 10000)
...
test_settings_max_frame_size_too_large :: proc(t: ^testing.T) {
	...
	// Too large (> 2^24 - 1)
	err := http2.settings_apply_remote(&ctx, .MAX_FRAME_SIZE, 16777216)
...
test_settings_ack_tracking :: proc(t: ^testing.T) {
	...
	http2.settings_mark_local_acked(&ctx)
	testing.expect(t, ctx.local_settings_acked == true, "Should mark local acked")

	http2.settings_mark_remote_acked(&ctx)
...
test_settings_getters :: proc(t: ^testing.T) {
	...
	http2.settings_apply_local(&ctx, .INITIAL_WINDOW_SIZE, 100000)
	http2.settings_apply_remote(&ctx, .INITIAL_WINDOW_SIZE, 200000)
	http2.settings_apply_remote(&ctx, .MAX_FRAME_SIZE, 32768)
	http2.settings_apply_remote(&ctx, .MAX_CONCURRENT_STREAMS, 50)

	testing.expect(t, http2.settings_get_local_window_size(&ctx) == 100000, "Should get local window")
	testing.expect(t, http2.settings_get_remote_window_size(&ctx) == 200000, "Should get remote window")
	testing.expect(t, http2.settings_get_remote_max_frame_size(&ctx) == 32768, "Should get max frame size")
	testing.expect(t, http2.settings_get_remote_max_concurrent_streams(&ctx) == 50, "Should get max streams")
}
...
test_settings_can_push :: proc(t: ^testing.T) {
	...
	testing.expect(t, http2.settings_can_push(&ctx) == true, "Client push enabled by default")

	http2.settings_apply_remote(&ctx, .ENABLE_PUSH, 0)
	testing.expect(t, http2.settings_can_push(&ctx) == false, "Push should be disabled")
}
...
test_settings_build_frame :: proc(t: ^testing.T) {
	...
	frame, ok := http2.settings_build_frame(&ctx)
...
test_settings_build_ack_frame :: proc(t: ^testing.T) {
	frame := http2.settings_build_ack_frame()

	testing.expect(t, frame.header.type == .SETTINGS, "Should be SETTINGS frame")
	testing.expect(t, frame.header.stream_id == 0, "Should be on stream 0")
	testing.expect(t, frame.header.length == 0, "ACK should have no payload")
	testing.expect(t, (frame.header.flags & http2.SETTINGS_FLAG_ACK) != 0, "Should have ACK flag")
...
test_settings_is_ack :: proc(t: ^testing.T) {
	...
	frame, _ := http2.settings_build_frame(&ctx)
	defer delete(frame.settings)
	testing.expect(t, http2.settings_is_ack(&frame) == false, "Should not be ACK")

	// ACK frame
	ack_frame := http2.settings_build_ack_frame()
	testing.expect(t, http2.settings_is_ack(&ack_frame) == true, "Should be ACK")
}
...
test_settings_max_header_list_size :: proc(t: ^testing.T) {
	...
	err := http2.settings_apply_remote(&ctx, .MAX_HEADER_LIST_SIZE, 16384)
	testing.expect(t, err == .None, "Should apply successfully")
